...
"""
    ...
    environment {
      ...
      container_name = "ma-dog-sources-service"
    }
    ...
    stages {
        ...
        stage('Deploy Helm') {
          ...
          steps {
            container('helm') { 
                 checkout scm
                 sh "helm upgrade -i ${container_name} ./k8s/ma-services --namespace default --set image.repository=${registry},image.tag=${container_name}-$BUILD_NUMBER,service.port=5000 --wait"
               } 
          }
        }
    }
